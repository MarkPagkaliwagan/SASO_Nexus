...
export default function ExamCreate() {
  ...
  return (
    <div className="min-h-screen w-full p-6 text-slate-900">
      <div className="mx-auto w-full  space-y-6">

        {/* Exam Header */}
        <div className="rounded-2xl bg-white shadow-md overflow-hidden">
          <div className="border-b border-gray-200 p-4 bg-gradient-to-r from-emerald-950 via-emerald-900 to-emerald-800">
            <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
              <div className="flex-1 space-y-2">
                <input
                  ...
                  className="w-full rounded-md border border-gray-200 bg-white p-3 text-xl font-semibold placeholder-slate-400 focus:ring-2 focus:ring-yellow-300"
                />
                <input
                  placeholder="Short description (optional)"
                  value={description}
                  onChange={(e) => setDescription(e.target.value)}
                  className="w-full rounded-md border border-gray-100 bg-white p-2 text-sm placeholder-slate-400 focus:ring-1 focus:ring-yellow-200"
                />
              </div>
              <button
                onClick={() => addQuestion(null, "text")}
                className="inline-flex items-center gap-2 rounded-lg bg-green-600 px-5 py-2 text-sm font-medium text-white hover:bg-green-700 shadow"
              >
                <FiPlus /> Add Question
              </button>
            </div>
          ...
          {/* Horizontal Exam Settings + Save */}
          <div className="w-full mx-auto mt-4 border border-gray-200 p-4 shadow-sm bg-white flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 rounded-md">

            {/* Left: Exam settings title + toggle */}
            <div className="flex items-center gap-6">
              <div className="text-sm font-semibold text-green-800">Exam settings</div>
            ...
            <div className="flex items-center gap-6 text-sm text-gray-700">
              <div>Total questions: <strong className="text-green-800">{totalQuestionsCount}</strong></div>
              <div>Total points: <strong className="text-green-800">{totalPoints}</strong></div>
            </div>

            {/* Right: Validate + Save buttons */}
            <div className="flex items-center gap-2 flex-wrap">
              <button
                onClick={() => {
                  const invalid = sectionsEnabled
                    ? sections.some((sec) => sec.questions.some((q) => q.answers.every((a) => !a.isCorrect)))
                    : questions.some((q) => q.answers.every((a) => !a.isCorrect));
      ...
      {/* All Exams - Modern Dashboard Table */}
      <div className="w-full mx-auto mt-8 rounded-2xl bg-white border border-gray-200 p-6 shadow-lg">

        <h2 className="text-2xl font-semibold mb-6 text-green-800">All Exams</h2>

        <div className="overflow-x-auto">
          <table className="min-w-full text-sm divide-y divide-gray-200">
            <thead className="bg-yellow-50 text-green-800">
              <tr>
            ...
            <tbody className="bg-white divide-y divide-gray-100">
              {exams.map((exam) => (
                <tr key={exam.id} className="hover:bg-green-50 transition-colors">
                  <td className="px-6 py-3 font-medium text-green-700">{exam.title}</td>
                  <td className="px-6 py-3 text-gray-700">{exam.description || "—"}</td>
                  <td className="px-6 py-3 text-gray-600">{new Date(exam.created_at).toLocaleDateString()}</td>
                  <td className="px-6 py-3">
                    <a href={`/exam/${exam.id}`} className="text-green-600 hover:underline">View</a>
                  </td>
                  <td className="px-6 py-3">
                    ...
                    <label className="relative inline-flex items-center cursor-pointer">
                      <input
                        type="checkbox"
                        checked={exam.status === "open"}
                        onChange={() => toggleStatus(exam.id)}
                        className="sr-only peer"
                      />
                  ...
                  <td className="px-6 py-3 flex gap-2">
                    <button
                      ...
                      className="px-3 py-1 text-xs rounded bg-red-500 text-white font-medium hover:bg-red-600 transition-colors"
                    >
                      Delete
                    </button>
                  </td>
              ...
              {exams.length === 0 && (
                <tr>
                  <td colSpan="6" className="px-6 py-6 text-center text-gray-400">No exams found.</td>
                </tr>
              )}
            </tbody>
          </table>
