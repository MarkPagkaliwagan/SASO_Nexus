...
const LABELS = {
  ...
  verbal_rs: "Verbal RS",
  verbal_pc: "Verbal PC",
  nonverbal_rs: "Nonverbal RS",
  nonverbal_pc: "Nonverbal PC",
  overall_rs: "Overall RS",
  overall_pc: "Overall PC",
...
function StatCard({title, value, subtitle}){
  return (
    <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 6 }}
      whileHover={{ y: -4 }} className={`rounded-lg p-4 border ${THEME.panel} shadow-sm border-gray-100`}>
...
export default function GSAnalyticsAdvanced(){
  ...
  useEffect(() => {
    ...
    axios.get("/api/gs-academic-years")
      .then(res => setAcademicYearsList(res.data?.years || []))
  ...
  const [histField, setHistField] = useState("overall_rs");

  useEffect(()=>{
    setLoadingLevels(true);
    axios.get("/api/gs-analytics")
      .then(res=>{
        const list = (res.data && res.data.grade_levels) || [];
        setGradeLevels(list);
        if (list.length && !selectedLevel) setSelectedLevel(list[0].gradeLevel);
      })
  ...
  function fetchDetail(level, pageNum=1, year=""){
    ...
    axios.get(`/api/analytics/gs/${encodeURIComponent(level)}?${params.toString()}`)
      .then(res => setDetail(res.data))
      .catch(err => { console.error("load gs detail", err); setDetail(null); })
  ...
  function toggleCol(col){ setVisibleCols(prev => prev.includes(col) ? prev.filter(c=>c!==col) : [...prev, col]); }
  ...
  function exportCSV(){
    ...
    const headers = [
      "id","nameFamily","nameGiven","nameMiddle","gender","address","academicYear","gradeLevel","last_school_name",
      ...NUMERIC_FIELDS,
    ...
    saveAs(blob, `gs_${selectedLevel}_page${page}_${new Date().toISOString().slice(0,10)}.csv`);
  }
  ...
  function exportExcel(){
    ...
    if (!rows.length) return alert("No rows to export.");
    const headers = [
      "ID","Last Name","First Name","Middle","Gender","Address","Academic Year","Grade Level","Last School",
      ...NUMERIC_FIELDS.map(f=>LABELS[f] || f),
      "Remarks","Payment Type"
    ];
    const data = rows.map(r => [
      r.id,r.nameFamily,r.nameGiven,r.nameMiddle,r.gender,r.address,r.academicYear,r.gradeLevel,r.last_school_name,
      ...NUMERIC_FIELDS.map(f=>r[f]),
      r.remarks, r.payment_type
    ]);
    const ws = XLSX.utils.aoa_to_sheet([headers, ...data]);
    ws['!cols'] = headers.map(() => ({ wch: 18 }));
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "GS Data");
    XLSX.writeFile(wb, `gs_${selectedLevel}_page${page}_${new Date().toISOString().slice(0,10)}.xlsx`);
  }
  ...
  function exportTopNCSV(){
    if (!detail) return alert("No data.");
    const rows = [...(detail.rows||[])].map(r=>({ ...r, __rankKey: safeNumber(r.overall_rs) || 0 })).sort((a,b)=>b.__rankKey - a.__rankKey).slice(0, topN);
    if (!rows.length) return alert("No rows to export.");
    const headers = ["Rank","ID","LastName","FirstName","Middle","Score"];
    const csv = [headers.join(",")];
    rows.forEach((r,i)=>{
      const score = r.overall_rs ?? r.overall_pc ?? "";
      const line = [i+1, r.id, r.nameFamily, r.nameGiven, r.nameMiddle, score].map(v=>`"${String(v||"").replace(/,/g,"")}"`).join(",");
      csv.push(line);
    });
    const blob = new Blob([csv.join("\n")], { type: "text/csv" });
    saveAs(blob, `${(detail.gradeLevel||selectedLevel||"gs").replace(/\s+/g,"_")}_top_${topN}_${new Date().toISOString().slice(0,10)}.csv`);
  }
    ...
    <div className={`${THEME.bg1} min-h-screen py-8 px-4 text-black`}>
      <div className="w-full mx-auto">
        <header className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          <div>
            <h1 style={{ color: THEME.greenDark }} className="text-3xl md:text-4xl font-extrabold tracking-tight">Grade School Analytics</h1>
            <p className="text-sm text-gray-600 mt-1">Interactive analytics — <span className="font-medium">Grade School</span>. <span className="text-xs text-gray-400">(UI tailored)</span></p>
          </div>

          <div className="flex flex-wrap gap-3 w-full md:w-auto items-center">
            <div className="flex items-center gap-2">
              <select aria-label="Select GS grade level" className="px-3 py-2 rounded-lg border bg-white shadow-sm"
                value={selectedLevel} onChange={(e) => { setSelectedLevel(e.target.value); setPage(1); }}>
                {loadingLevels ? <option>Loading...</option> : null}
                {!loadingLevels && gradeLevels.length === 0 ? <option>No grade levels</option> : null}
                {!loadingLevels && gradeLevels.map((c) => (
                  <option key={c.gradeLevel} value={c.gradeLevel}>
                    {c.gradeLevel} — {c.totalApplicants}
                  </option>
                ))}
              </select>

              <select aria-label="Academic year" className="px-3 py-2 border rounded-lg shadow-sm w-44"
                value={academicYear} onChange={(e) => { setAcademicYear(e.target.value); setPage(1); }}>
                <option value="">All years</option>
                {loadingYears ? <option>Loading...</option> : null}
                {academicYearsList.map(y => <option key={y} value={y}>{y}</option>)}
              </select>

              <button title="Refresh data" onClick={() => { setPage(1); setQuery(""); setSortBy({ key: "nameFamily", dir: "asc" }); fetchDetail(selectedLevel, 1, academicYear); }}
        ...
        <AnimatePresence>
          {detail && (
            <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
              <StatCard title="Grade level" value={detail.gradeLevel} subtitle={`Total applicants: ${detail.total}`} />
              <StatCard title="Overall (avg)" value={detail.averages?.overall_rs_avg ?? "—"} subtitle={`Overall PC avg: ${detail.averages?.overall_pc_avg ?? "—"}`} />
              <StatCard title="Verbal (avg)" value={detail.averages?.verbal_rs_avg ?? "—"} subtitle={`Verbal PC avg: ${detail.averages?.verbal_pc_avg ?? "—"}`} />
         ...
         {detail && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <motion.div initial={{opacity:0, y:8}} animate={{opacity:1, y:0}} className="bg-white rounded-lg p-4 shadow-sm border border-gray-100 min-h-[320px] h-full flex flex-col">
                    ...
                    {paymentData.length === 0 ? (
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        <ComposedChart data={paymentData} margin={{ top: 10, right: 20, left: 0, bottom: 30 }}>
                          ...
                          <Tooltip formatter={(value, name) => name === "percent" ? [`${value}%`, "Percentage"] : [value, "Count"]} />
                          <Legend verticalAlign="bottom" height={36} />
                          <Bar yAxisId="left" dataKey="value">
                            {paymentData.map((entry, index) => (<Cell key={index} fill={entry.color} />))}
                          </Bar>
                          <Line yAxisId="right" type="monotone" dataKey="percent" stroke="#f59e0b" strokeWidth={2} dot={{ r: 4 }} />
                        </ComposedChart>
                      </ResponsiveContainer>
                    )}
                  </div>
            ...
            {detail && (
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2 gap-6 mb-6">
                {[
                  ...
                  { title: "Remarks", data: remarksData, type: "bar" }
                  ...
                  <div key={chart.title} className="bg-white rounded-lg p-4 shadow-sm border border-gray-100 min-h-[340px] flex flex-col">
                    ...
                    {chart.data.length === 0 ? (
                      ...
                      <div className="flex-1">
                        <ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={chart.data} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={60} label={({ name, value }) => `${name} (${value})`}>
                          {chart.data.map((entry, index) => (<Cell key={`cell-${index}`} fill={entry.color} />))}
                        </Pie><Tooltip /><Legend verticalAlign="bottom" height={36} /></PieChart></ResponsiveContainer>
                      </div>
                    ) : chart.type === "bar" ? (
                      <div className="flex-1"><ResponsiveContainer width="100%" height="100%"><BarChart data={chart.data}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="name" tick={{ fontSize: 10 }} /><YAxis /><Tooltip /><Bar dataKey="value">{chart.data.map((entry, index) => (<Cell key={`cell-${index}`} fill={entry.color} />))}</Bar></BarChart></ResponsiveContainer></div>
                    ) : chart.type === "bar-horizontal" ? (
                      <div className="flex-1"><ResponsiveContainer width="100%" height="100%"><BarChart layout="vertical" data={chart.data}><CartesianGrid strokeDasharray="3 3" /><XAxis type="number" /><YAxis dataKey="name" type="category" tick={{ fontSize: 10 }} /><Tooltip /><Bar dataKey="value">{chart.data.map((entry, index) => (<Cell key={`cell-${index}`} fill={entry.color} />))}</Bar></BarChart></ResponsiveContainer></div>
                    ) : chart.type === "line" ? (
                      <div className="flex-1"><ResponsiveContainer width="100%" height="100%"><LineChart data={chart.data}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="name" /><YAxis /><Tooltip /><Line type="monotone" dataKey="value" stroke={THEME.green} strokeWidth={2} /></LineChart></ResponsiveContainer></div>
                    ) : chart.type === "area" ? (
                      <div className="flex-1"><ResponsiveContainer width="100%" height="100%"><AreaChart data={chart.data}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="name" /><YAxis /><Tooltip /><Area type="monotone" dataKey="value" stroke={THEME.green} fill={THEME.green} /></AreaChart></ResponsiveContainer></div>
                    ) : chart.type === "scatter" ? (
                      <div className="flex-1"><ResponsiveContainer width="100%" height="100%"><ScatterChart><CartesianGrid /><XAxis dataKey="name" type="category" /><YAxis dataKey="value" /><Tooltip cursor={{ strokeDasharray: '3 3' }} /><Scatter data={chart.data} fill={THEME.green} /></ScatterChart></ResponsiveContainer></div>
                    ) : null}
                  </div>
                ))}
              </div>
            ...
            {/* Rankings & histogram, table, modal (kept similar) */}
            {detail && (
                ...
                <div className="flex flex-col md:flex-row gap-4">
                  <div className="w-full md:w-64 p-2 border rounded">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button onClick={()=>navigator.clipboard.writeText(JSON.stringify(detail.rows || []))} className="px-3 py-1 rounded border">Copy</button>
                    </div>
                  ...
                  <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="p-2 border rounded">
                      <div className="text-sm font-medium mb-2">VC/VR vs Overall</div>
                      {detail.rows.length===0 ? <div className="text-xs text-gray-500">No data</div> : (
                        <ResponsiveContainer width="100%" height={180}>
                          <ScatterChart>
                            <CartesianGrid />
                            <XAxis dataKey="vc_rs" name="VC RS" />
                            <YAxis dataKey="overall_rs" name="Overall RS" />
                            <Tooltip cursor={{ strokeDasharray: '3 3' }} />
                            <Scatter data={(detail.rows||[]).map(r=>({ vc_rs: safeNumber(r.vc_rs), overall_rs: safeNumber(r.overall_rs), name: toFullName(r) })).filter(p=>p.vc_rs!==null && p.overall_rs!==null)} fill={COLOR_PALETTE[0]} />
                          </ScatterChart>
                        </ResponsiveContainer>
                      )}
                    </div>
                    ...
                    <div className="md:col-span-2 p-2 border rounded">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <div className="text-xs text-gray-500">Sorted by overall_rs</div>
                      </div>
                      <div className="max-h-56 overflow-auto">
                        <table className="w-full text-sm">
                          ...
                          <tbody>
                            {([...detail.rows].sort((a,b)=>(safeNumber(b.overall_rs)||0)-(safeNumber(a.overall_rs)||0)).slice(0, topN)).map((r,i)=> (
                              <tr key={r.id} className="border-b hover:bg-amber-50">
                                <td className="p-1 text-xs">{i+1}</td>
                                <td className="p-1 text-xs">{toFullName(r)}</td>
                                <td className="p-1 text-xs font-medium">{r.overall_rs ?? r.overall_pc ?? '—'}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
            ...
            {detail && (
              <div className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-x-auto">
                <div className="p-3 border-b text-sm text-gray-600 flex flex-col md:flex-row items-start md:items-center justify-between gap-3">
                  <div className="flex items-center gap-2">
                    <input aria-label="Search" value={query} onChange={(e) => debouncedSetQuery(e.target.value)} placeholder="Search name / remarks / payment / year" className="px-3 py-2 border rounded w-64 shadow-sm" />
                  ...
                  <div className="flex items-center gap-2">
                    <select className="px-2 py-1 border rounded text-sm" value={sortBy.key} onChange={(e)=>onSort(e.target.value)}>
                      ...
                      <option value="overall_rs">Overall RS</option>
                      <option value="overall_pc">Overall PC</option>
                    </select>
                    <button onClick={() => setSortBy(s => ({ key: s.key, dir: s.dir === "asc" ? "desc" : "asc" }))} className="px-2 py-1 border rounded text-sm">
                      Dir: {sortBy.dir}
                    </button>
                  </div>
                </div>
                  ...
                  <thead className="bg-gradient-to-r from-white to-amber-50 sticky top-0 z-10 shadow-sm">
                    <tr>
                      ...
                      {allCols.filter(c => visibleCols.includes(c)).map(col => (
                        <th key={col} className="p-3 text-center font-semibold text-black whitespace-nowrap w-[140px] truncate">
...
// helper function used inside component (moved here for clarity)
function prepareCategoryChartData(stats){
  return Object.entries(stats || {}).map(([k,v], i)=>({
    name: k,
    value: v,
