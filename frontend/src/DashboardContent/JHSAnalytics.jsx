...
export default function JHSAnalyticsAdvanced(){
  ...
  useEffect(() => {
    ...
    axios.get("/api/jhs-academic-years")
  ...
  function fetchDetail(level, pageNum=1, year=""){
    ...
    axios.get(`/api/analytics/jhs/${encodeURIComponent(level)}?${params.toString()}`)
      .then(res => setDetail(res.data))
      .catch(err => { console.error("load jhs detail", err); setDetail(null); })
  ...
  function exportCSV(){
    ...
    saveAs(blob, `jhs_${selectedLevel}_page${page}_${new Date().toISOString().slice(0,10)}.csv`);
  }
  ...
  function exportExcel(){
    ...
    XLSX.utils.book_append_sheet(wb, ws, "JHS Data");
    XLSX.writeFile(wb, `jhs_${selectedLevel}_page${page}_${new Date().toISOString().slice(0,10)}.xlsx`);
  }
  ...
  function exportTopNCSV(){
    ...
    const blob = new Blob([csv.join("\n")], { type: "text/csv" });
    saveAs(blob, `${(detail.gradeLevel||selectedLevel||"jhs").replace(/\s+/g,"_")}_top_${topN}_${new Date().toISOString().slice(0,10)}.csv`);
  }
        ...
        <header className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          ...
          <div className="flex flex-wrap gap-3 w-full md:w-auto items-center">
            <div className="flex items-center gap-2">
              <select aria-label="Select JHS grade level" className="px-3 py-2 rounded-lg border bg-white shadow-sm"
