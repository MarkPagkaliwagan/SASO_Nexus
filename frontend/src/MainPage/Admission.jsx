...
export default function Admission() {
  ...
  return (
    <div
      className="relative min-h-screen flex flex-col items-center  p-4 sm:p-8 space-y-8 bg-fixed bg-cover bg-center"
      style={{
        backgroundImage: "url('/src/images/Campus2.jpg')",
      }}
    >
      <div className="absolute inset-0 bg-[#003332]/50 bg-opacity-10 z-0"></div>

      <div className="relative z-10 w-full flex flex-col items-center space-y-6 max-w-7xl">
        {/* Header + single Apply Now button */}
        <motion.div
          className="w-full flex flex-col sm:flex-row mt-11 items-center justify-between gap-4"
          initial={{ opacity: 0, y: -20 }}
          animate={{ opacity: 1, y: 0 }}
        >
          <motion.h2
            className="text-3xl sm:text-4xl font-extrabold text-center text-white tracking-tight"
          ...
          {/* Single Apply Now button (top) */}
          <motion.button
            onClick={handleApplyClick}
            whileHover={{ scale: 1.03 }}
            whileTap={{ scale: 0.97 }}
            className="mt-2 sm:mt-0 bg-blue-600 hover:bg-blue-700 text-white px-5 py-3 rounded-lg font-semibold shadow-lg"
          >
            Apply Now
          </motion.button>
        </motion.div>
        ...
        <div className="max-w-4xl mx-auto w-full">
          <p className="flex items-start text-gray-700 text-sm sm:text-base bg-green-50 border border-green-200 rounded-lg p-4">
            ...
            After submitting your application form, you are required to pay 200 pesos at the cashier and bring your receipt on the day of your scheduled exam. This exam will be conducted onsite using a computer-based system.
          </p>
        </div>

        {/* SINGLE-COLUMN LIST (hover to expand) */}
          ...
          <div className="grid grid-cols-1 gap-6 w-full">
            {programs.map((program, index) => (
              <motion.div
                ...
                className={`flex flex-col md:flex-row items-center text-left border rounded-2xl p-6 shadow-md bg-white cursor-pointer transition-all duration-300 hover:shadow-lg
                  ${expandedIndex === index ? "bg-green-50 border-green-600 scale-[1.01] shadow-xl" : ""}`}
              >
                <img
                  src={program.img}
                  alt={program.name}
                  className="w-20 h-20 sm:w-24 sm:h-24 object-contain mb-4 md:mb-0 md:mr-6"
                />
                <div className="flex-1 w-full">
                  ...
                  <AnimatePresence>
                    {expandedIndex === index && (
                      ...
                      >
                        <ul className="list-disc list-inside text-sm sm:text-base mb-0 whitespace-pre-line">
                        ...
                        {/* NOTE: No Apply Now inside each detail (single top Apply Now used) */}
                      </motion.div>
                    )}
                  </AnimatePresence>
                </div>
        ...
        {/* Additional Info Cards (unchanged functionality) */}
        <motion.div
          className="w-full max-w-6xl p-6 rounded-2xl shadow-xl border border-yellow-300 bg-yellow-50 transform perspective-1000 hover:scale-105 hover:rotate-1 hover:shadow-2xl transition-all duration-500"
        ...
        >
          <div className="relative z-10 space-y-6">
            ...
            {[ /* array of sections same as before */ 
              {
                title: "For Freshmen",
                content: (
        ...
        {/* On-site Registration Procedures card (unchanged) */}
        <motion.div
          className="w-full max-w-6xl p-6 rounded-2xl shadow-xl border border-yellow-300 bg-yellow-50 transform perspective-1000 hover:scale-105 hover:rotate-1 hover:shadow-2xl transition-all duration-500"
          initial={{ opacity: 0, y: -30 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        <AnimatePresence>
          {showModal && (
            ...
            >
              <motion.div
                className="bg-yellow-50 rounded-2xl p-6 max-w-xl w-full relative shadow-2xl transform perspective-1000"
              ...
              >
                ...
                <motion.button
                  ...
                  className={`w-full text-white py-2 rounded-lg font-semibold transition-all duration-300 ${
                    agreed ? "bg-yellow-700 hover:bg-yellow-800" : "bg-gray-400 cursor-not-allowed"
                  }`}
                  disabled={!agreed}
