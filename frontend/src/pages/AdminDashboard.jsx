...
import { CheckCircle, BarChart2, Users, FileText, Home, Activity, Clipboard, User, Menu, X, Bell, Moon, Sun } from 'react-feather';
...
export default function AdminDashboard() {
  ...
  const menuItems = [
    ...
    { icon: darkMode ? <Sun size={18} /> : <Moon size={18} />, label: 'Dark Mode', action: toggleDarkMode }
  ];

  const SidebarContent = ({ isMobile }) => (
    <div className="flex flex-col h-full w-full">
      <div className="p-4 border-b border-green-800 flex items-center gap-3">
        <div className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center text-sm font-bold">SP</div>
        <AnimatePresence>
          {(isExpanded || isMobile) && (
    ...
    <div className="min-h-screen flex flex-col md:flex-row overflow-hidden transition-colors duration-300">
        ...
        {activePanel === 'notification' && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}>
            <div className="p-6 rounded-xl shadow-lg transition-colors duration-300 bg-white dark:bg-gray-800 text-black dark:text-white">
              <h2 className="text-xl font-bold mb-4 text-[rgb(6,73,26)] dark:text-green-300">Notifications</h2>
              <p className="text-gray-700 dark:text-gray-200">No new notifications at the moment.</p>
            </div>
          </motion.div>
        )}
      </main>
      ...
      {showPopup && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ duration: 0.3 }} className="rounded-xl shadow-lg p-6 md:p-8 max-w-md w-full text-center bg-white dark:bg-gray-800 text-black dark:text-white">
            <CheckCircle size={48} className="text-green-600 dark:text-green-300 mx-auto mb-4" />
            <h2 className="text-xl font-bold truncate text-[rgb(6,73,26)] dark:text-green-300">Login Verified</h2>
            <p className="text-gray-600 dark:text-gray-200 mt-2">You are now logged in as authorized SASO Admin.</p>
            <button onClick={handleClosePopup} className="mt-6 bg-green-600 hover:bg-green-700 text-white px-6 py-2 rounded transition">OK</button>
          </motion.div>
        </div>
      )}
    </div>
