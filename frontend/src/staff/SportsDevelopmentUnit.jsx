...
// ✅ Updated imports (keep only needed panels)
...
export default function StaffDashboard() {
  ...
  const SidebarContent = ({ isMobile }) => {
    ...
    const itemHover = !isTouch ? " hover:text-black hover:bg-yellow-400" : "";
    const dropdownItemBase =
      "text-sm text-left px-2 py-1 rounded w-full text-left";
    const dropdownItemHover = !isTouch ? " hover:bg-yellow-400/60" : "";
    const logoutHover = !isTouch ? " hover:bg-yellow-500" : "";

    return (
      <div className="flex flex-col h-full w-full">
        {/* 🔹 Logo Section */}
        <div className="p-4 border-b border-yellow-400/30 flex items-center gap-3">
    ...
    <div className="min-h-screen flex flex-col md:flex-row overflow-hidden bg-gradient-to-r from-slate-100 via-neutral-50 to-slate-200">
      ...
      <div className="md:hidden fixed top-0 left-0 w-full flex justify-between items-center bg-yellow-700 text-white z-50 h-14 px-4 shadow-md">
        <div className="flex items-center gap-3 truncate">
          ...
          <div className="text-lg font-semibold truncate">Sports Dev. Staff</div>
        </div>
        ...
        {activePanel === "Dashboard" && (
          <div className="relative w-full min-h-screen flex flex-col items-center justify-start text-center overflow-hidden pt-32 md:pt-40">
            <div className="relative z-10 flex flex-col items-center justify-center gap-8">
              ...
              <h1 className="text-6xl md:text-8xl font-extrabold text-yellow-700 tracking-tight">
                SAN PABLO COLLEGES
              </h1>

              <h2 className="text-3xl md:text-5xl font-semibold text-gray-700">
                Department of Sports Development Unit
              </h2>

              <span className="mt-4 text-2xl md:text-3xl font-medium text-yellow-800">
                Staff Dashboard
              </span>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          ...
          >
            ...
            <p className="text-gray-600 mt-2">
              You are now logged in as authorized Sports Development Staff.
            </p>
            <button
